...
4. **Binary download** — fetches `fuzz-up` binary + signature from `asymmetric-research/fuzz-up` releases via `gh`
5. **Signature verification** — uses cosign to verify the binary against the repo's `cosign.pub`
...
- The action pins `sigstore/cosign-installer` by commit SHA.
